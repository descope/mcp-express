...
// Mock fetch
const mockFetch = jest.fn().mockImplementation(() => Promise.resolve(new Response()));
...
describe('registrationHandler', () => {
    ...
    describe('successful registration', () => {
        ...
        it('should register a client with valid metadata', async () => {
            ...
            expect(clientInfo).toHaveProperty('client_name', validClientMetadata.client_name);
            expect(clientInfo).toHaveProperty('redirect_uris', validClientMetadata.redirect_uris);
            expect(clientInfo).toHaveProperty('logo_uri', validClientMetadata.logo_uri);

            // Verify fetch calls
            expect(mockFetch).toHaveBeenCalledTimes(2);
            expect(mockFetch.mock.calls[0][0]).toBe('https://api.descope.com/v1/mgmt/thirdparty/app/create');
            expect(mockFetch.mock.calls[1][0]).toBe('https://api.descope.com/v1/mgmt/thirdparty/app/load?id=test-app-id');
        });
    });
