...
describe('metadataHandler', () => {
    ...
    it('should return correct metadata structure', async () => {
        ...
        expect(metadata).toHaveProperty('issuer', 'https://api.descope.com/test-project');
        expect(metadata).toHaveProperty('authorization_endpoint', 'https://test.example.com/authorize');
        expect(metadata).toHaveProperty('token_endpoint', 'https://api.descope.com/oauth2/v1/apps/token');
        expect(metadata).toHaveProperty('revocation_endpoint', 'https://api.descope.com/oauth2/v1/apps/revoke');
        expect(metadata).toHaveProperty('registration_endpoint', 'https://test.example.com/register');
        expect(metadata).toHaveProperty('response_types_supported', ['code']);
        expect(metadata).toHaveProperty('code_challenge_methods_supported', ['S256']);
        expect(metadata).toHaveProperty('token_endpoint_auth_methods_supported', ['client_secret_post']);
        expect(metadata).toHaveProperty('grant_types_supported', ['authorization_code', 'refresh_token']);
        expect(metadata).toHaveProperty('revocation_endpoint_auth_methods_supported', ['client_secret_post']);
    ...
    it('should include service documentation URL when provided', async () => {
        ...
        const metadata = response.body;
        expect(metadata).toHaveProperty('service_documentation', 'https://docs.example.com');
    });
