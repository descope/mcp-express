...
describe('authorizationHandler', () => {
    ...
    describe('GET requests', () => {
        it('should redirect to Descope authorization URL with query parameters', async () => {
            ...
            expect(redirectUrl.searchParams.get('redirect_uri')).toBe('https://test.example.com/callback');
        ...
        it('should properly encode URL parameters', async () => {
            const response = await request(app)
                ...
                .query({
                    response_type: 'code',
                    client_id: 'test-client',
                    redirect_uri: 'https://test.example.com/callback?param=value&special=!@#$%^&*()',
                    scope: 'openid profile'
                });

            expect(response.status).toBe(302);
            const redirectUrl = new URL(response.header.location);
            expect(redirectUrl.searchParams.get('redirect_uri')).toBe('https://test.example.com/callback?param=value&special=!@#$%^&*()');
        });
    });
}); 