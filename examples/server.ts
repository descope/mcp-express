...
import { userInfoTool } from "./tools/userInfoTool.js";
import { externalApiTool } from "./tools/externalApiTool.js";
...
const mcpRouter = descopeMcpAuthRouter((server) => {
  ...
  greetingTool(server);
  userInfoTool(server);
  externalApiTool(server);
  weatherTool(server);

  console.log("✅ Registered MCP tools:");
  console.log("  - greeting: Say hello to authenticated users");
  console.log("  - user_info: Get authenticated user information");
  console.log("  - external_api: Demonstrate outbound token usage");
  console.log("  - weather: Get weather data using external API");
}, provider);
